...
test "fuzzy_match_examples" {
  inspect(fuzzy_match("swt", "ss_ww_tt"), content="true")
  inspect(fuzzy_match("fb", "foo_bar"), content="true")  
  inspect(fuzzy_match("xyz", "concatenate"), content="false")
}
```
...
test "fuzzy_score_examples" {
  let score1 = fuzzy_score("test", "test_file")  // 32 - start + consecutive
  let score2 = fuzzy_score("tf", "test_file")    // 16 - word boundaries  
  let score3 = fuzzy_score("xyz", "test_file")   // -1 - no match
...
test "case_insensitive" {
  inspect(fuzzy_match_ignore_case("API", "ApiController"), content="true")
  let score = fuzzy_score_ignore_case("MOON", "MoonBit")
...
test "vscode_comprehensive" {
  // Basic usage
  let (score, positions) = vscode_fuzzy_score("swt", "ss_ww_tt", true)
  inspect(score, content="66")
  inspect(positions, content="[1, 3, 6]")
  
  // Camel case matching
  let (score2, positions2) = vscode_fuzzy_score("fB", "fooBar", true)  
  inspect(score2, content="41")
  inspect(positions2, content="[0, 3]")
  
  // Exact match
  let (score3, positions3) = vscode_fuzzy_score("test", "test", true)
...
test "scoring_examples" {
  ...
  let (score1, _) = vscode_fuzzy_score("abc", "abc", true)      // 69 - all consecutive
  let (score2, _) = vscode_fuzzy_score("abc", "aXbXc", true)    // 63 - with gaps
  
  // Word boundaries are highly valued
  let (score3, _) = vscode_fuzzy_score("fb", "foo_bar", true)   // 39 - separator bonus
  let (score4, _) = vscode_fuzzy_score("tF", "testFile", true)  // 47 - camelCase bonus
...
test "performance_test" {
  ...
  let (score, positions) = vscode_fuzzy_score("test", long_text, true)
...
test "unicode_support" {
  // Emoji support
  let (score1, _pos1) = vscode_fuzzy_score("🌙", "moon🌙bit", true)
  inspect(score1 > 0, content="true")
  
  // CJK characters  
  let (score2, _pos2) = vscode_fuzzy_score("月", "moon月bit", true)
  inspect(score2 > 0, content="true")
  
  // Accented characters
  let (score3, _pos3) = vscode_fuzzy_score("café", "café_file", true)
...
test "file_search_example" {
  ...
  let results = files.map(fn(file) {
    let (score, positions) = vscode_fuzzy_score("main", file, true)
...
test "command_palette_example" {
  ...
  let matches = commands.map(fn(cmd) {
    let (score, positions) = vscode_fuzzy_score(query, cmd.to_lower(), true)
    (cmd, score, positions)
  }).filter(fn(result) { result.1 > 0 })
...
test "algorithm_comparison" {
  ...
  // Simple algorithm
  let simple_score = fuzzy_score(pattern, text)
  let simple_match = fuzzy_match(pattern, text)
  
  // VS Code algorithm  
  let (vscode_score, positions) = vscode_fuzzy_score(pattern, text, true)
